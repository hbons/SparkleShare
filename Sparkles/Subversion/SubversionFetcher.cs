...
namespace Sparkles.Subversion {
    ...
    public class GitFetcher : SSHFetcher {
        ...
        protected override bool IsFetchedRepoEmpty {
            get {
                var git_rev_parse = new SubversionCommand (TargetFolder, "rev-parse HEAD");
        ...
        public override bool Fetch ()
        {
            ...
            git_clone = new SubversionCommand (Configuration.DefaultConfiguration.TmpPath,
        ...
        public override string Complete (StorageType selected_storage_type)
        {
            ...
            if (IsFetchedRepoEmpty) {
                ...
                var git_add    = new SubversionCommand (TargetFolder, "add .sparkleshare");
                var git_commit = new SubversionCommand (TargetFolder, "commit --message=\"Initial commit by SparkleShare\"");
                ...
                if (selected_storage_type == StorageType.LargeFiles) {
                    var git_branch = new SubversionCommand (TargetFolder, "branch x-sparkleshare-lfs", auth_info);
                    git_branch.StartAndWaitForExit ();
                }

                if (selected_storage_type == StorageType.Encrypted) {
                    var git_branch = new SubversionCommand (TargetFolder,
                ...
                var git_show_ref = new SubversionCommand (TargetFolder,
                ...
                var git_checkout = new SubversionCommand (TargetFolder, string.Format ("checkout --quiet --force {0}", branch));
        ...
        public override void EnableFetchedRepoCrypto (string password)
        {
            ...
            var git_config_required = new SubversionCommand (TargetFolder, "config filter.encryption.required true");

            var git_config_smudge = new SubversionCommand (TargetFolder, "config filter.encryption.smudge " +
                string.Format ("\"openssl enc -d -aes-256-cbc -base64 -S {0} -pass file:{1}\"", password_salt, password_file));

            var git_config_clean = new SubversionCommand (TargetFolder, "config filter.encryption.clean " +
        ...
        public override bool IsFetchedRepoPasswordCorrect (string password)
        {
            ...
            if (!File.Exists (password_check_file_path)) {
                var git_show = new SubversionCommand (TargetFolder, "show HEAD:.sparkleshare");
        ...
        {
            var git_ls_remote = new SubversionCommand (Configuration.DefaultConfiguration.TmpPath,
        ...
        void InstallConfiguration ()
        {
            ...
            foreach (string setting in settings) {
                var git_config = new SubversionCommand (TargetFolder, "config " + setting);
        ...
        void InstallGitLFS ()
        {
			var git_config_required = new SubversionCommand (TargetFolder, "config filter.lfs.required true");

            string GIT_SSH_COMMAND = SubversionCommand.FormatGitSSHCommand (auth_info);
            ...
            var git_config_smudge = new SubversionCommand (TargetFolder,
                string.Format ("config filter.lfs.smudge \"{0}\"", smudge_command));

            var git_config_clean = new SubversionCommand (TargetFolder,
